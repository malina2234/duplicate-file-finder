...
1.  Откройте терминал.
2.  Создать папку и перейти в нее, куда хотите сохранить проект ( потом в этой папке нажать на правую клавишу тачпада и открыть терминал).
![img_3.png](img_3.png)
...
3.  Выполните команду `git clone`, чтобы скопировать проект с GitHub ( просто скопируйте с этого поля текст):
    ...
    ```
4.  В этом же терминале выполните. Перейдите в только что созданную папку проекта. **Все дальнейшие команды выполняются из нее.** Не закрывайте этот терминал пока что 
    ```bash
