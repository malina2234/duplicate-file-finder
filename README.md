...
4.  Выполните команду `git clone`, чтобы скопировать проект с GitHub:
    ```bash
    git clone https://github.com/malina2234/duplicate-file-finder.git
    ```
5.  Перейдите в только что созданную папку проекта. **Все дальнейшие команды выполняются из нее.**
    ```bash
